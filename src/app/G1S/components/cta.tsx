      ...
      <div className="container mx-auto px-4 md:px-0 max-w-7xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4 items-start">
          ...
          <div className="text-black  ">
            <h2 className="text-4xl sm:text-5xl font-britti font-bold">{t("g1s.cta.title")}</h2>
            <p className="mt-6 text-gray-900/90 text-lg">{t("g1s.cta.discountLine")}</p>
            <p className="mt-4 text-gray-900/90">{t("g1s.cta.joinFounders")}</p>
            ...
            <div className="mt-8 ">
              <div className="bg-black text-white rounded-xl p-5 sm:p-6 shadow-lg grid grid-cols-12 items-stretch gap-8 sm:gap-10">
                ...
                <div className="md:ml-8 col-span-9 sm:col-span-10 leading-snug pl-2 sm:pl-4">
                  <div className="text-sm text-gray-100">{t("g1s.cta.price.regularLabel")}</div>
                  <div className="relative inline-block mt-1">
                    <span className="text-lg text-white">{t("g1s.cta.price.regularValue")}</span>
                    <span
                      aria-hidden
                      className="absolute left-0 right-0 top-1/2 -translate-y-1/2 block h-[2px] bg-red-500 -rotate-[14deg] scale-x-100 rounded"
                    />
                  </div>
                  <div className="text-xs text-gray-100 mt-3">{t("g1s.cta.price.untilLabel")}</div>
                  <div className="text-4xl sm:text-5xl font-bold mt-1">{t("g1s.cta.price.discountValue")}</div>
                </div>
              </div>
            </div>
